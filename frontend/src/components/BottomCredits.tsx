...
const BottomCredits: React.FC<BottomCreditsProps> = ({ areaName, mode = 'overlay' }) => {
  ...
  if (mode === 'inline') {
    return (
      <div className={`text-white text-sm md:text-base px-3 py-2 rounded shadow select-none transition-all duration-700 ${entered ? 'bg-black/45 opacity-100 translate-y-0' : 'bg-black/45 opacity-0 translate-y-2'} ${settled ? 'opacity-75' : ''}`}>
        {content}
      </div>
    );
  }
